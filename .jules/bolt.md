...
## 2023-11-20 - [Selenium DOM extraction vs Image Loading]
**Learning:** Selenium IPC roundtrips (like finding elements and accessing properties sequentially) can be a massive bottleneck on pages with many elements.
**Action:** When scraping bulk static data from DOM elements, prioritize using `driver.execute_script()` to query the DOM via native JavaScript and return all data in a single IPC call instead of iterating `find_element` in Python.
