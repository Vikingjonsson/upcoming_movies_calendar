...
def scrape_movie_detail_page(
    ...
    try:
        ...
        try:
            # ⚡ Bolt: Fetch plot and poster in a single JS execution to avoid slow IPC roundtrips
            js_script = """
            ...
            if data and data.get("plot") is not None:
                plot_description = data["plot"]
            else:
                logging.warning("Could not fetch description for '%s'", movie.title)

            if data and data.get("poster") is not None:
