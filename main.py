...
def collect_movie_links_from_calendar_page(
    ...
    # ⚡ Bolt: Use bulk JavaScript extraction instead of multiple slow find_element calls
    js_script = """
    const calendarSections = document.querySelectorAll(arguments[0]);
    const movies = [];

    for (const section of calendarSections) {
        const releaseDateElement = section.querySelector('.' + arguments[1]);
        if (!releaseDateElement) continue;
        const releaseDateText = releaseDateElement.textContent.trim();

        const movieEntries = section.querySelectorAll(arguments[2]);
        for (const entry of movieEntries) {
            const titleElement = entry.querySelector('.' + arguments[3]);
            if (titleElement) {
                const title = titleElement.textContent.trim();
                const url = titleElement.href; // Use .href for absolute URL

                if (title && url && releaseDateText) {
                    movies.push({
                        title: title,
