...
def collect_movie_links_from_calendar_page(
    ...
    # ⚡ Bolt: Bulk extract DOM data via JavaScript to eliminate slow IPC roundtrips
    # from multiple Python Selenium .find_element() and .text calls.
    script = f"""
        const sections = document.querySelectorAll(`{CALENDAR_SECTION_SELECTOR}`);
        const movies = [];
        for (const section of sections) {{
            const releaseDateElement = section.querySelector(`.{RELEASE_DATE_CLASS_NAME}`);
            if (!releaseDateElement) continue;
            const releaseDateText = releaseDateElement.innerText.trim();
    ...
    logging.info(
        "Found %d movies on calendar page via bulk extraction", len(movie_links)
    )
    return movie_links
