...
def scrape_movie_detail_page(
    ...
    try:
        ...
        try:
            ...
            pass  # Continue to extraction anyway

        # ⚡ Bolt: Bulk fetch detail page elements via JavaScript to eliminate IPC overhead
        # from multiple synchronous find_element and .text calls.
        js_script = """
        ...
        result = driver.execute_script(js_script, PLOT_SELECTOR, POSTER_IMAGE_SELECTOR)

        if result.get("plot") is not None:
            plot_description = result["plot"]
        else:
            logging.warning("Could not fetch description for '%s'", movie.title)
            plot_description = DEFAULT_DESCRIPTION

        if result.get("poster") is not None:
            poster_image_url = result["poster"]
        else:
