...
def collect_movie_links_from_calendar_page(
    ...
    # ⚡ Bolt: Fetch movie links in bulk via JavaScript instead of using Selenium's slow
    # find_element and .text methods which trigger IPC overhead for every single element.
    js_script = f"""
    ...
    for (const section of sections) {{
        ...
        const releaseDate = dateElement.innerText.trim();
        ...
        for (const entry of entries) {{
            ...
            if (title && url && releaseDate) {{
                movies.push({{title: title, release_date_text: releaseDate, imdb_url: url.trim()}});
            }}
        }}
    }}
    return movies;
    ...
    returned_data = driver.execute_script(js_script)
    logging.info("Found %d calendar sections", len(calendar_sections))

    movie_links = [
        ScheduledMovie(
            title=str(movie_dict.get("title")),
            release_date_text=str(movie_dict.get("release_date_text")),
            imdb_url=str(movie_dict.get("imdb_url")),
        )
        for movie_dict in returned_data
    ]
