...
def collect_movie_links_from_calendar_page(
    ...
    # ⚡ Bolt: Use JS to extract all movies in a single IPC call to avoid N+1 roundtrips
    js_script = f"""
            ...
            for (const entry of entries) {{
                ...
                if (titleElement && titleElement.href) {{
                    movies.push({{
                        ...
                        // use .href instead of getAttribute to get absolute URL
                        imdb_url: titleElement.href.trim()
                    }});
                }}
            }}
        }}
    ...
    logging.info(
        "Found %d calendar sections (implicitly via JS)", len(extracted_movies)
    )  # Not sections but movies now

    movie_links: list[ScheduledMovie] = []
    for m in extracted_movies:
        if m.get("title") and m.get("release_date_text") and m.get("imdb_url"):
            movie_links.append(
                ScheduledMovie(
                    title=m["title"],
                    release_date_text=m["release_date_text"],
                    imdb_url=m["imdb_url"],
                )
            )
