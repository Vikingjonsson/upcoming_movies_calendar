...
def collect_movie_links_from_calendar_page(
    ...
    # ⚡ Bolt: Optimize by replacing multiple synchronous Selenium calls with a single JS script.
    # This prevents hundreds of slow IPC roundtrips to the browser.
    ...
    for (const section of sections) {
        ...
        if (!dateEl) continue;
        const release_date_text = dateEl.textContent.trim();
        ...
        for (const entry of entries) {
            ...
            if (titleEl) {
                ...
                const href = titleEl.href; // Note: element.href resolves to absolute URL automatically in JS
                if (title && href && release_date_text) {
                    movies.push({
                        title: title,
