...
def collect_movie_links_from_calendar_page(
    ...
    # Wait for the main sections to be present before executing JS
    ...
    # ⚡ Bolt: Use execute_script to fetch data in bulk via JavaScript instead of making
    # multiple synchronous find_element or .text calls, to minimize slow IPC roundtrips.
    js_script = """
    ...
    for (const section of sections) {
        ...
        for (const entry of entries) {
            ...
            if (titleEl) {
                ...
                const url = titleEl.href; // Note: element.href resolves to absolute URL
                if (title && url) {
                    result.push({
                        title: title,
    ...
    for movie_data in raw_movies:
        movie_links.append(
            ScheduledMovie(
                ...
                release_date_text=movie_data["release_date_text"],
