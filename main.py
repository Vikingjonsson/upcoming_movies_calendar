...
def collect_movie_links_from_calendar_page(
    ...
    # Ensure at least one section is loaded before running script
    element_wait.until(
        EC.presence_of_element_located((By.CSS_SELECTOR, CALENDAR_SECTION_SELECTOR))
    )

    # ⚡ Bolt: Use execute_script for bulk DOM extraction to replace 150+ synchronous Selenium IPC roundtrips
    # with a single JS call. This resolves the N+1 find_element performance bottleneck.
    extraction_script = f"""
        const sections = document.querySelectorAll('{CALENDAR_SECTION_SELECTOR}');
        const movies = [];
        for (const section of sections) {{
            const dateEl = section.querySelector('.{RELEASE_DATE_CLASS_NAME}');
            if (!dateEl) continue;
            const dateText = dateEl.innerText.trim();

            const entries = section.querySelectorAll('{MOVIE_ENTRY_SELECTOR}');
            for (const entry of entries) {{
                const titleEl = entry.querySelector('.{MOVIE_TITLE_CLASS_NAME}');
                if (titleEl && titleEl.href) {{
                    movies.push({{
                        title: titleEl.innerText.trim(),
                        release_date_text: dateText,
                        imdb_url: titleEl.href
                    }});
                }}
            }}
        }}
    ...
    movie_data_list = driver.execute_script(extraction_script)
    logging.info("Found %d movies via bulk extraction", len(movie_data_list))

    movie_links: list[ScheduledMovie] = []
    for data in movie_data_list:
        if data["title"] and data["release_date_text"] and data["imdb_url"]:
            movie_links.append(
                ScheduledMovie(
                    title=data["title"],
    ...
    logging.info("Successfully mapped %d movies", len(movie_links))
    return movie_links
