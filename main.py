...
def collect_movie_links_from_calendar_page(
    ...
    # ⚡ Bolt: Bulk extract DOM data with execute_script to avoid slow IPC roundtrips
    # using .innerText for exact Selenium element.text match and .href for absolute URL resolution
    script = """
    const sections = document.querySelectorAll(arguments[0]);
    const movies = [];
    for (const section of sections) {
        const dateElements = section.getElementsByClassName(arguments[1]);
        if (dateElements.length === 0) continue;
        const releaseDate = dateElements[0].innerText.trim();

        const entries = section.querySelectorAll(arguments[2]);
        for (const entry of entries) {
            const titleElements = entry.getElementsByClassName(arguments[3]);
