...
def collect_movie_links_from_calendar_page(
    ...
    # ⚡ Bolt: Fetch entire movie list in bulk via JavaScript instead of hundreds of synchronous IPC roundtrips
    script = f"""
        ...
        for (const section of sections) {{
            ...
            const movieEntries = section.querySelectorAll('{MOVIE_ENTRY_SELECTOR}');
            for (const entry of movieEntries) {{
                const titleElement = entry.querySelector('.{MOVIE_TITLE_CLASS_NAME}');
                if (titleElement) {{
                    const title = titleElement.textContent.trim();
                    const href = titleElement.href; // Native .href resolves to absolute URL

                    if (title && href && releaseDateText) {{
                        movies.push({{
                            title: title,
    ...
    movie_dicts = driver.execute_script(script)
