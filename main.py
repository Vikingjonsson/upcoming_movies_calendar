...
def collect_movie_links_from_calendar_page(
    ...
    # ⚡ Bolt: Use execute_script to fetch all movies at once to avoid N+1 IPC roundtrips
    # We use element.href instead of getAttribute('href') to get absolute URLs
    script = f"""
    const movies = [];
    ...
    raw_movies = driver.execute_script(script)
    movie_links = [
        ScheduledMovie(
            title=m["title"],
