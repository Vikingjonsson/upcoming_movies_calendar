...
def collect_movie_links_from_calendar_page(
    ...
    # ⚡ Bolt: Bulk fetch movie links via JS to avoid hundreds of slow Selenium IPC roundtrips
    # using driver.execute_script instead of multiple synchronous find_elements and .text calls.
    # Note: Use element.href instead of element.getAttribute('href') to get absolute URLs.
    # Note: Use element.innerText instead of element.textContent for accurate text extraction.
    js_script = f"""
        ...
        return movies;
    """
    extracted_movies = driver.execute_script(js_script)
    ...
    for m in extracted_movies:
        movie_links.append(
            ScheduledMovie(
                ...
                release_date_text=m["release_date_text"],  # type: ignore
                imdb_url=m["imdb_url"],  # type: ignore
            )
        )
