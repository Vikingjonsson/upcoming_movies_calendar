...
def collect_movie_links_from_calendar_page(
    ...
    # ⚡ Bolt: Bulk extract DOM data via JS to avoid O(N) slow Selenium find_element/get_attribute IPC calls
    js_script = """
        ...
        for (const section of sections) {
            ...
            for (const entry of entries) {
                ...
                if (titleEl && titleEl.href && titleEl.innerText.trim()) {
                    results.push({
    ...
    movie_links = [ScheduledMovie(**item) for item in raw_results]
