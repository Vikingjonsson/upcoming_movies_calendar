...
def collect_movie_links_from_calendar_page(
    ...
    # Wait for at least one calendar section to be present to ensure the page has loaded
    element_wait.until(
    ...
    # ⚡ Bolt: Use JS to extract all movies in a single IPC call instead of hundreds of synchronous Selenium queries
    js_extraction_script = """
    const sections = document.querySelectorAll(arguments[0]);
    const results = [];
    for (const section of sections) {
        const dateEl = section.querySelector('.' + arguments[3]);
        if (!dateEl) continue;
        const dateText = dateEl.textContent.trim();
        const movies = section.querySelectorAll(arguments[1]);
        for (const movie of movies) {
            const titleEl = movie.querySelector('.' + arguments[2]);
            if (titleEl && titleEl.href && titleEl.textContent) {
                results.push({
