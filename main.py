...
def collect_movie_links_from_calendar_page(
    ...
    # ⚡ Bolt: Wait for the calendar sections to exist before executing bulk extraction script
    element_wait.until(
    ...
    # ⚡ Bolt: Use a single Javascript execution to bulk-extract data,
    # significantly reducing Selenium IPC overhead.
    bulk_extraction_script = f"""
    const sections = document.querySelectorAll('{CALENDAR_SECTION_SELECTOR}');
    const movies = [];

    sections.forEach(section => {{
        const dateElement = section.querySelector('.{RELEASE_DATE_CLASS_NAME}');
        if (!dateElement) return;
        const releaseDate = dateElement.textContent.trim();

        const entries = section.querySelectorAll('{MOVIE_ENTRY_SELECTOR}');
        entries.forEach(entry => {{
            const titleElement = entry.querySelector('.{MOVIE_TITLE_CLASS_NAME}');
            if (titleElement) {{
                // Note: using property .href directly gives absolute URL instead of .getAttribute('href')
                const url = titleElement.href;
                const title = titleElement.textContent.trim();

                if (url && title && releaseDate) {{
                    movies.push({{
                        title: title,
    ...
    scraped_data = driver.execute_script(bulk_extraction_script)

    movie_links = [
        ScheduledMovie(
            title=item["title"],
            release_date_text=item["release_date_text"],
            imdb_url=item["imdb_url"].strip(),
        )
        for item in scraped_data
    ]

    logging.info(
        "Found %d movies on calendar page via JS bulk extraction", len(movie_links)
    )
    return movie_links
