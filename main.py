...
def collect_movie_links_from_calendar_page(
    ...
    # ⚡ Bolt: Use execute_script for bulk extraction to avoid O(N) IPC roundtrips
    # This significantly improves performance when parsing large lists of DOM elements.
    script = """
    const movies = [];
    ...
    for (const section of sections) {
        ...
        if (!dateEl) continue;
        const releaseDateText = dateEl.innerText.trim();

        const entries = section.querySelectorAll(arguments[2]);
        for (const entry of entries) {
            const titleEl = entry.querySelector('.' + arguments[3]);
            if (titleEl && titleEl.href && titleEl.innerText) {
                // Using .href DOM property to get the absolute URL
                movies.push({
